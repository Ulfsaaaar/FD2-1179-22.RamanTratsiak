...
function preload() {
  ...
  S_Hit_01      = loadSound("assets/S_Hit_01.wav"); S_Hit_01.playMode("restart"); 
  S_Hit_02      = loadSound("assets/S_Hit_02.wav"); S_Hit_02.playMode("restart"); 
  S_Hit_03      = loadSound("assets/S_Hit_03.wav"); S_Hit_03.playMode("restart"); 
  S_Hit_04      = loadSound("assets/S_Hit_04.wav"); S_Hit_04.playMode("restart"); 
  S_Laser       = loadSound("assets/S_Laser.wav");
  S_Start_01    = loadSound("assets/S_Start_01.wav"); 
  S_Start_02    = loadSound("assets/S_Start_02.wav"); 
}
...
function keyPressed() {
  ...
  return false; // предотвращение поведение по дефолту
}
...
function Paddle() {
  ...
  this.cooldown = 30; // 1/2 секунды
  this.frame;         // стартовый кадр
  ...
  this.shoot = function() {
    if(this.laser && !this.onCooldown) {
      ...
      S_Laser.play();
      lasers.push(new Laser(this.x + (  PADDLE.w/4), this.y)); // левый лазер
      lasers.push(new Laser(this.x + (3*PADDLE.w/4), this.y)); // правый лазер
...
function Ball(x, y, dx, dy) {
      ...
      for(var i = bricks.length - 1; i > -1; i--) {
        ...
        if(pt) {
          ...
          if(bricks[i].hp == 0) {
            ...
            // 8% шанс выпадения улучшения
            if(random() > 0.92 && bricks[i].type != 2) powerUps.push(new PowerUp(bricks[i].x, bricks[i].y));
            bricks.splice(i, 1);
            S_Hit_02.play(); // нормальное столкновение с блоком
          } else {
            S_Hit_03.play(); // столкновение с металическим блоком
          }
        }
      }
  ...
  this.hitBrick = function(pt) {
    ...
    // изменение скорости полета мяча
...
function Brick(x, y, type) {
  ...
  this.type = type;
       ...
       if(this.type == 1) this.hp = Infinity;  // золотой блок
  else if(this.type == 2) this.hp = 2;         // серебрянный блок
  else                    this.hp = 1;         // цветной блок
